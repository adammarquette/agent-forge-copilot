...
/// <see cref="AnthropicQaFixture"/> LLM provider, and a real <see cref="McpToolDispatcher"/> in
/// front of the real QA OpenEMR deployment via <see cref="McpToolServerQaFixture"/>'s wiring.
/// Nothing here is mocked (tests/AGENTS.md): this is the only tier that proves the full loop -
/// model plans a tool call, the real dispatcher executes it against real FHIR data, the result
/// serializes back onto the real Anthropic wire format, and the model produces a final answer.
/// </summary>
public sealed class AgentOrchestratorQaFixture
{
    ...
    public AgentOrchestratorQaFixture()
    {
        ...
        Orchestrator = new AgentOrchestrator(llm.Provider, dispatcher, NullLogger<AgentOrchestrator>.Instance);
    }
}
