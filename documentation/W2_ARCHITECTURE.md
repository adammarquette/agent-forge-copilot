...
- **Data access (W2-D14):** entities, `DbContext`, and schema live in a shared
  **`AgentForge.Data`** project. **EF Core** (Npgsql provider) + **`Pgvector.EntityFrameworkCore`**
...
| **W2-D14** | Shared `AgentForge.Data` project — EF Core + Pgvector.EntityFrameworkCore for entities / vector / index / KNN; raw SQL for RRF; EF Migrations for schema + scripts | Dapper / raw Npgsql only; per-project DbContexts | ORM-native for entities, migrations, and dense KNN; raw SQL only where hybrid fusion needs it; one migrations home; EF provider major pinned to the EF Core 10 line in CPM (the pgvector-EF floor won't force it) |
