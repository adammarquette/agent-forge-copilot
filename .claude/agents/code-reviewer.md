...
- **Report to the durable record, not to your caller.** On a PR, post a **review body** whose first line is
  `**Verdict: Approve**` or `**Verdict: Request changes**`, naming the head SHA you reviewed, via
  `bash .github/scripts/post-verdict.sh review <pr> COMMENT <body-file>` — exit 0 is the only outcome that
  means you ruled. A PR comment or an inline note looks right and counts for nothing: the gate reads reviews,
  so it hangs the author's watcher instead (`documentation/CI-SETUP.md` §8). On a working diff with no PR,
  use `ReportFindings`. A verdict handed back only to the reviewed party lets the reviewed decide what the
  review said.
